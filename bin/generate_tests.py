...
import roc_utils
from roc_utils import TypeJSON
...
def load_canonical(exercise: str, spec_path: Path, test_opts: TestsTOML) -> TypeJSON:
    ...
    spec["properties"] = roc_utils.get_tested_properties(spec)
    return spec
...
def generate_exercise(
    ...
    try:
        ...
        tests_path = exercise / f"{roc_utils.to_kebab(slug)}-test.roc"
        spec["has_error_case"] = roc_utils.has_error_case(spec["cases"])
...
def generate(
    ...
    **_,
    ...
    # Register filters and tests from roc_utils
    env.filters["escape_invalid_escapes"] = roc_utils.escape_invalid_escapes
    env.filters["parse_datetime"] = roc_utils.parse_datetime
    env.filters["regex_find"] = roc_utils.regex_find
    env.filters["regex_replace"] = roc_utils.regex_replace
    env.filters["regex_split"] = roc_utils.regex_split
    env.filters["RocDict"] = roc_utils.RocDict
    env.filters["RocTag"] = roc_utils.RocTag
    env.filters["to_camel"] = roc_utils.to_camel
    env.filters["to_pascal"] = roc_utils.to_pascal
    ...
    env.filters["to_roc_tag"] = roc_utils.to_roc_tag
    env.filters["to_roc_tuple"] = roc_utils.to_roc_tuple
    env.filters["to_roc"] = roc_utils.to_roc
    env.filters["to_snake"] = roc_utils.to_snake
    env.filters["wrap_overlong"] = roc_utils.wrap_overlong
    env.filters["zip"] = zip
    env.globals["current_date"] = datetime.now(tz=timezone.utc).date()
    env.tests["error_case"] = roc_utils.error_case
