ZebraPuzzle :: {}.{
	...
	owns_zebra : Try(Person, [NotFound])
	owns_zebra = owner_of(|house| house.animal == 5)

	drinks_water : Try(Person, [NotFound])
	drinks_water = owner_of(|house| house.drink == 5)
}
...
owner_of : (House -> Bool) -> Try(Person, [NotFound])
owner_of = |condition| {
	houses = solution?
	house = houses.find_first(condition)?
	match house.person {
		1 => Ok(Englishman)
		2 => Ok(Spaniard)
		3 => Ok(Ukrainian)
...
solution : Try(List(House), [NotFound])
solution = {
	num_houses = 5 # rule 1
				...
				|state, value| {
					...
					if updated_houses->is_valid() {
						if updated_houses.len() == num_houses and field_index == 4 {
							...
							match find_first_solution(next_houses, next_house, next_field_index) {
...
set_field = |house, field_index, value| {
	match field_index {
		...
		_ => {
			crash "field_index should always be between 0 and 4"
		}
	}
}
...
rule3 = |houses| {
	same_house(houses, (|h| h.person, 2), (|h| h.animal, 1))
}

# The person in the green house drinks coffee.
rule4 : List(House) -> Bool
rule4 = |houses| {
	same_house(houses, (|h| h.color, 2), (|h| h.drink, 1))
}

# The Ukrainian drinks tea.
rule5 : List(House) -> Bool
rule5 = |houses| {
	same_house(houses, (|h| h.person, 3), (|h| h.drink, 2))
}

# The green house is immediately to the right of the ivory house.
rule6 : List(House) -> Bool
rule6 = |houses| {
	green_house = houses.find_first_index(|house| house.color == 2)
	ivory_house = houses.find_first_index(|house| house.color == 3)
	match (green_house, ivory_house) {
...
rule7 = |houses| {
	same_house(houses, (|h| h.animal, 2), (|h| h.activity, 1))
}

# The person in the yellow house is a painter.
rule8 : List(House) -> Bool
rule8 = |houses| {
	same_house(houses, (|h| h.color, 4), (|h| h.activity, 2))
}
...
rule9 = |houses| {
	houses
		.map_with_index(|house, index| (house, index))
		.all(
			|(house, index)| {
...
rule11 = |houses| {
	reader_house = houses.find_first_index(|house| house.activity == 3)
	fox_house = houses.find_first_index(|house| house.animal == 3)
	match (reader_house, fox_house) {
...
rule12 = |houses| {
	painter_house = houses.find_first_index(|house| house.activity == 2)
	horse_house = houses.find_first_index(|house| house.animal == 4)
	match (painter_house, horse_house) {
		(Ok(painter_index), Ok(horse_index)) => (painter_index == horse_index + 1) or (horse_index == painter_index + 1)
		_ => Bool.True
	}
}

# The football person drinks orange juice.
rule13 : List(House) -> Bool
rule13 = |houses| {
	same_house(houses, (|h| h.activity, 4), (|h| h.drink, 4))
}

# The Japanese plays chess.
rule14 : List(House) -> Bool
rule14 = |houses| {
	same_house(houses, (|h| h.person, 5), (|h| h.activity, 5))
}

# The Norwegian lives next to the blue house.
rule15 : List(House) -> Bool
rule15 = |houses| {
	norwegian_house = houses.find_first_index(|house| house.person == 4)
	blue_house = houses.find_first_index(|house| house.color == 5)
	match (norwegian_house, blue_house) {
		(Ok(norwegian_index), Ok(blue_index)) => (norwegian_index == blue_index + 1) or (blue_index == norwegian_index + 1)
		_ => Bool.True
	}
}

# There must be no duplicates (e.g., two green houses)
	...
	[|h| h.activity, |h| h.animal, |h| h.color, |h| h.drink, |h| h.person]
		.all(
			|field| {
				...
				values.len() == Set.from_list(values).len()
			},
		)
}
