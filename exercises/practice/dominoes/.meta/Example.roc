        ...
        find_chain_helper = |used, available| {
            match available {
                [] => {
                    match used {
                        [] => Ok([])
                        [single] => if single.0 == single.1 Ok(used) else Err(NoChainExists)
                        [first, .., last] => if first.0 == last.1 Ok(used) else Err(NoChainExists)
                    }
                }
