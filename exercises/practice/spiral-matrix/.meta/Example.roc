...
spiral_matrix = \size ->
    ...
    |> List.walk { x: -1, y: 0, dx: 1, dy: 0, matrix: zero_matrix } \state, index ->
        get_value = \x_i64, y_i64 ->
            row = state.matrix |> List.get (Num.toU64 y_i64) |> Result.withDefault []
            row |> List.get (Num.toU64 x_i64)

        (dx, dy) =
            (nx, ny) = (state.x + state.dx, state.y + state.dy)
            if nx < 0 || ny < 0 || get_value nx ny != Ok 0 then
