...
game_tree =
    P.const (\node_props -> \alternatives -> build_game_node node_props alternatives)
    |> P.skip (S.codeunit '(')
...
sub_tree =
    ...
    |> P.keep
        (
            P.oneOf [
                P.const (\t -> t) |> P.keep (P.lazy (\_ -> game_tree)),
...
property =
    P.map2 prop_ident (P.oneOrMore prop_value) \id, values -> (
