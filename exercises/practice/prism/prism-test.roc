...
# https://github.com/exercism/problem-specifications/tree/main/exercises/prism/canonical-data.json
# File last updated on 2026-09-22

import Prism exposing [find_sequence]

# zero prisms
expect {
	start = { x: 0, y: 0, angle: 0 }
	prisms = []
	result = find_sequence({ start, prisms })
	result == []
}
...
expect {
	...
	prisms = [{ id: 3, x: 0, y: -10, angle: 0 }, { id: 1, x: -10, y: 0, angle: 0 }, { id: 2, x: 10, y: 0, angle: 0 }]
	result = find_sequence({ start, prisms })
	result == []
}

# going down zero hits
expect {
	start = { x: 0, y: 0, angle: -90 }
	prisms = [{ id: 1, x: 10, y: 0, angle: 0 }, { id: 2, x: 0, y: 10, angle: 0 }, { id: 3, x: -10, y: 0, angle: 0 }]
	result = find_sequence({ start, prisms })
	result == []
}

# going left zero hits
expect {
	start = { x: 0, y: 0, angle: 180 }
	prisms = [{ id: 2, x: 0, y: 10, angle: 0 }, { id: 3, x: 10, y: 0, angle: 0 }, { id: 1, x: 0, y: -10, angle: 0 }]
	result = find_sequence({ start, prisms })
	result == []
}
...
expect {
	start = { x: 0, y: 0, angle: -180 }
	prisms = [{ id: 1, x: 0, y: -10, angle: 0 }, { id: 2, x: 0, y: 10, angle: 0 }, { id: 3, x: 10, y: 0, angle: 0 }]
	result = find_sequence({ start, prisms })
	result == []
}
...
# upward refraction two hits
expect {
	start = { x: 0, y: 0, angle: 0 }
	prisms = [{ id: 1, x: 10, y: 10, angle: 0 }, { id: 2, x: 10, y: 0, angle: 90 }]
	result = find_sequence({ start, prisms })
	result == [2, 1]
}

# downward refraction two hits
expect {
	start = { x: 0, y: 0, angle: 0 }
	prisms = [{ id: 1, x: 10, y: 0, angle: -90 }, { id: 2, x: 10, y: -10, angle: 0 }]
	result = find_sequence({ start, prisms })
	result == [1, 2]
}
...
expect {
	start = { x: 0, y: 0, angle: 0 }
	prisms = [{ id: 2, x: 10, y: 0, angle: 0 }, { id: 1, x: 20, y: 0, angle: -180 }]
	result = find_sequence({ start, prisms })
	result == [2, 1, 2]
}
